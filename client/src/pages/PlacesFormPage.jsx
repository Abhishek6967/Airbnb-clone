...
const PlacesFormPage = () => {
  ...
  useEffect(  () => {
    ...
    setLoading(true);
     axios.get(`http://localhost:8000/places/${id}`).then((response) => {
  ...
  const savePlace = async (e) => {
    ...
    if (id) {
      ...
      const { data } = await axios.put('http://localhost:8000/places/update-place', {
        id,
        ...placeData,
      });
    } else {
      ...
      const { data } = await axios.post('http://localhost:8000/places/add-places', placeData, {
        headers: {
